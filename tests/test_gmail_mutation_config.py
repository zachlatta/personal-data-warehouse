...
from personal_data_warehouse.config import CONTACTS_SCOPE, GMAIL_COMPOSE_SCOPE, GMAIL_MODIFY_SCOPE, load_settings
...
def test_gmail_auth_cli_passes_include_gmail_modify_flag(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(
        gmail_auth,
        "run_oauth_flow",
        lambda email_address, *, include_gmail_modify, include_gmail_compose, include_contacts_write: calls.append(
            (email_address, include_gmail_modify, include_gmail_compose, include_contacts_write)
        )
        or {},
    )
    ...
    assert calls == [("zach@example.test", True, False, False)]
...
def test_gmail_auth_cli_passes_include_gmail_compose_flag(monkeypatch) -> None:
    ...
    assert calls == [("zach@example.test", False, True, False)]
...
def test_gmail_auth_cli_passes_include_contacts_write_flag(monkeypatch) -> None:
    ...
    assert calls == [("zach@example.test", False, False, True)]
