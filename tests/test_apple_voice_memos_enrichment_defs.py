...
def test_apple_voice_memos_enrichment_drains_all_history_in_bounded_batches() -> None:
    assert DEFAULT_VOICE_MEMOS_ENRICHMENT_BATCH_SIZE == 10
    assert not hasattr(apple_voice_memos_enrichment_defs, "apple_voice_memos_enrichment_recorded_after")
...
def test_apple_voice_memos_enrichment_backlog_sensor_skips_when_backlog_is_empty(monkeypatch) -> None:
    ...
    assert "recorded_after" not in calls[0][1]
