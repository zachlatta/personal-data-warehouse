...
def _local(unique_id: str, recording_id: str, title: str, flags: int) -> LocalRecordingTitle:
    return LocalRecordingTitle(unique_id=unique_id, recording_id=recording_id, title=title, flags=flags)
...
def _sql_envelope(rows: list[dict]) -> dict:
    return {
        "data": {
            "column_names": ["recording_id", "title"],
            "total_rows": len(rows),
    ...
    session = FakeSession(
        _sql_envelope(
            [
                ...
                {"recording_id": "20260102 100000-BBBB2222", "title": "Vendor onboarding call"},
            ]
        )
    )
