...
# Optional one-time bootstrap for an empty database only. Prefer the --install flow below.
...
- WHOOP OAuth client secrets are env-only. The OAuth token lives in the warehouse's private
...
Configure client credentials and production database access, then authorize once in a browser and
install the result directly into the private credential authority:
...
```bash
uv run personal-data-warehouse-whoop-auth --manual --no-browser --install
```

`--install` writes the token directly to `private.whoop_oauth_tokens` without printing it. That
private row is the sole runtime authority. WHOOP rotates and invalidates both access and refresh
tokens on refresh, so every credential mutation—first bootstrap, scheduled or direct refresh, and
explicit reauthorization—takes the same database advisory lock. Refresh also holds the account row
lock from the provider request through the atomic database update. A stale racer waits and adopts
the stored winner rather than reusing the consumed refresh token. A legacy `WHOOP_TOKEN_JSON_B64`
value can populate an absent private row once; it can never overwrite an existing row, even after
`action_required`. Dagster's advisory lock remains an outer guard for whole scheduled syncs.

The provider boundary is inherently ambiguous: WHOOP can consume a single-use refresh token and
then return a 5xx or lose the response before the replacement commits. The client never resubmits
the same refresh token inside one process. A fresh Dagster retry either completes the rotation or
surfaces the credential as needing explicit reauthorization. To verify credentials without
syncing, run (validation still requires an env token because it intentionally does not mutate the
private store):
...
credentials, and Postgres URL are present at code-location load; otherwise it stays safely stopped.
...
fails the first no-progress Dagster run. Later schedule ticks for that exact fingerprint are
skipped, avoiding hundreds of duplicate failures while `/pipelines` remains in `attention`.
Repair by re-running the OAuth flow with
`uv run personal-data-warehouse-whoop-auth --install` from a trusted terminal that has production
database access; the next schedule tick sees the new private fingerprint and self-heals without a
deployment restart. Transient token-endpoint failures (5xx, network) still fail loudly. See the
[WHOOP OAuth operations runbook](docs/whoop-oauth-operations.md).
