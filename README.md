...
Every `timeline.events` row carries a `priority`. Its meanings and the intended
uses of the single scope parameter are generated from the warehouse catalog:
...
Scoping to priority is usually the difference between an answer and the whole
corpus; bulk traffic otherwise dominates many attention-oriented searches.
...
`gmail_mailbox_sync_every_five_minutes`, which runs the Gmail sync every 5 minutes.
Incremental (history-id) runs take a few minutes at most, so this cadence still leaves an
idle gap between runs instead of running back-to-back continuously (which once kept the
