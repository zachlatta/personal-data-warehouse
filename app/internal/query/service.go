...
func (s *Service) SchemaOverview(ctx context.Context) Response {
	...
	out.WriteString("-- Cross-source search: the searchable_text view unions EVERY searchable text column in the warehouse into (source, subsource, context, who, occurred_at, account, ref, text). Start broad — SELECT source, subsource, count(*), max(occurred_at) FROM searchable_text WHERE text ~* '\\mterm\\M' GROUP BY 1, 2 ORDER BY 3 DESC — then refine with ordinary WHERE clauses (e.g. exclude noisy public Slack: NOT (source = 'slack' AND subsource = 'public_channel')) and drill into the underlying table via ref for full rows or BM25 ranking. To find everything involving a person, first fuzzy-resolve them in the person_identities view (name %> 'their name' ORDER BY word_similarity('their name', name) DESC) to get email/slack_user_id/phone, then filter sources by those identifiers — most of a person's activity never contains their name as text. For meetings, read searchable_text source='transcript' subsources 'action_items' and 'summary' first: enrichment already extracted the commitments. Summaries are lossy, though — before reporting an email request as unanswered or a question as open, search the full transcript text (and Slack DMs) dated AFTER the request; decisions are often made on calls and appear only in raw transcripts.\n")
