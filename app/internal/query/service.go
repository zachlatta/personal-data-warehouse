...
func (s *Service) SchemaOverview(ctx context.Context) Response {
	...
	out.WriteString("-- Fast paths: latest rows should use the source's indexed timestamp column with ORDER BY ... DESC LIMIT N; source-specific text search should pass both max_results and sources into search_text (e.g. search_text('q', 5, sources => ARRAY['google_drive'])) or use one indexed column directly. For Slack thread/pair analysis, use a small conversation_id IN (...) list, a message_datetime range, and live human thread filters (is_deleted = 0, thread_ts <> '', user_id <> '', bot_id = '') before grouping slack_messages. For exact Gmail recipient lookup, include is_deleted = 0 and use the indexed recipient expression (to_addresses || cc_addresses || bcc_addresses) && ARRAY['person@example.test'] instead of casts or unnest.\n")
	out.WriteString("-- Agent-session fast paths: for cwd/worktree lookups, query agent_session_events directly with cwd = '...' or cwd ILIKE '%name%' and DISTINCT ON (source, session_id). Do not filter clean_agent_sessions by cwd in interactive queries; that view aggregates every event before applying the cwd filter.\n")
	out.WriteString("-- Never do this in interactive queries: SELECT COUNT(*)/COUNT(DISTINCT ...)/GROUP BY across all slack_messages; search_text(...) WHERE source = 'gmail' or 'slack' after the function call; outer LIMIT on search_text without lowering max_results; lower(coalesce(text,'')) LIKE ...; to_addresses::text ILIKE ...; unnest(to_addresses) for fuzzy search; or broad body_html/body_text scans without a date/source limit.\n")
