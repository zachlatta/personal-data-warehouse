...
func (s *Service) SchemaOverview(ctx context.Context) Response {
	...
	out.WriteString("-- General search flow: first use search.search_text() over timeline.events; then query the source table named by the timeline hit's source_table/source_pk for detailed rows, joins, attachments, thread context, etc. Source-table BM25 indexes were removed with the legacy fan-out search. For table-specific fuzzy/typo search after you know the table, use trigram indexes: WHERE col %> 'qery' ORDER BY word_similarity('qery', col) DESC LIMIT 20; use ILIKE for exact substrings.\n")
	out.WriteString("-- Cross-source search (the default way agents should do general search): SELECT * FROM search.search_text('offer letter', 50). It searches the unified timeline BM25 document and returns timeline hits as (source, subsource, context, who, occurred_at, account, ref, text, score), where ref is '<adapter>:<event_id>' for timeline.events. Scores are negative (more negative = better). Optional args: search.search_text(query, max_results, sources => ARRAY['slack','gmail'], since => '2026-03-01'). The `sources` filter takes the discoverable tokens from SELECT * FROM search.search_text_sources(): gmail, slack, slack_file, imessage, whatsapp, note, transcript, google_drive, calendar, contact, agent_session, mutation, mutation_request, warehouse. Detail text (attachments, media enrichments, Drive extracts, transcripts) is folded into the parent timeline event's search document; drill from timeline.events.source_table/source_pk for full source rows. An unknown token RAISES an error listing the valid set. Ranking caveats: BM25 terms are OR'd, stemmed WHOLE words (no phrase/AND match, no typo tolerance), so a noisy or short top-N does NOT mean a thing is absent. For 'find every mention of X', raise max_results, try the term alone and with context, and fall back to source-table trigram %> / ILIKE for name variants when you already know the table.\n")
