...
// download link covering the file ID and expiry.
func (s *Service) SignObjectDownload(fileID string, exp time.Time) string {
	return base64.RawURLEncoding.EncodeToString(s.objectDownloadMAC(fileID, exp.Unix()))
}

// VerifyObjectDownload checks the signature and expiry of a download link's
// exp/sig query parameters.
func (s *Service) VerifyObjectDownload(fileID, expRaw, sig string) error {
	if fileID == "" || sig == "" {
	...
	if !hmac.Equal(provided, s.objectDownloadMAC(fileID, expUnix)) {
...
func (s *Service) objectDownloadMAC(fileID string, expUnix int64) []byte {
