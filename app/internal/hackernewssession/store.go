// Package hackernewssession persists the news.ycombinator.com session cookie credential that
// the local `pdw hn publish-session` helper captures, so the server-side
// Dagster poller can authenticate to the Hacker News backend API.
//
// The table is the same hacker_news_sessions table the Python warehouse manages
// (see ensure_hacker_news_session_table in postgres.py); the CREATE here is the
// idempotent twin so the very first publish (before any poll) succeeds.
package hackernewssession
...
var createSchemaSQL = "CREATE SCHEMA IF NOT EXISTS " +
	warehouse.QuoteIdent(warehouse.SchemaOf("hacker_news_sessions"))

var createTableSQL = `
CREATE TABLE IF NOT EXISTS ` + warehouse.SQLRelation("hacker_news_sessions") + ` (
...
var ensureColumnSQL = []string{
	`ALTER TABLE ` + warehouse.SQLRelation("hacker_news_sessions") + ` ADD COLUMN IF NOT EXISTS expired_at timestamptz`,
	`ALTER TABLE ` + warehouse.SQLRelation("hacker_news_sessions") + ` ADD COLUMN IF NOT EXISTS expired_token_sha256 text NOT NULL DEFAULT ''`,
	`ALTER TABLE ` + warehouse.SQLRelation("hacker_news_sessions") + ` ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'ok'`,
	`ALTER TABLE ` + warehouse.SQLRelation("hacker_news_sessions") + ` ADD COLUMN IF NOT EXISTS error text NOT NULL DEFAULT ''`,
}

var upsertSQL = `
INSERT INTO ` + warehouse.SQLRelation("hacker_news_sessions") + ` (
...
func (s *PostgresStore) Upsert(ctx context.Context, account, sessionKey, sessionToken, sourceBrowser string, now time.Time) (Ack, error) {
	...
	if _, err := s.db.ExecContext(ctx, createTableSQL); err != nil {
		return Ack{}, fmt.Errorf("ensure hacker_news_sessions: %w", err)
	}
	for _, statement := range ensureColumnSQL {
		if _, err := s.db.ExecContext(ctx, statement); err != nil {
			return Ack{}, fmt.Errorf("migrate hacker_news_sessions health state: %w", err)
		}
	}
	...
	if _, err := s.db.ExecContext(ctx, upsertSQL,
		...
		return Ack{}, fmt.Errorf("upsert hacker_news_sessions: %w", err)
	}
	return Ack{
		Account:       account,
