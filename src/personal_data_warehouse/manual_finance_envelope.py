"""The manual-finance document metadata envelope (server-side copy).

The uploader itself is the Go `pdw ingest manual-finance` (app/internal/uploaders/
manualfinance); this module keeps the envelope shape and the provenance dedup sha
in Python for the server-side tools that re-post metadata for documents already
in the warehouse (scripts/refile_manual_finance_documents.py). The sha here MUST
stay byte-identical to the Go client's, or a re-file stops matching the original
claim.
