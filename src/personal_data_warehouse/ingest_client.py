"""Server-side client for the app's semantic ingestion endpoints (``POST /ingest/...``).

The device uploaders are Go now (``pdw ingest <source>``, app/internal/ingestclient
is the full client). What remains here is the half a *Dagster-hosted* client still
needs: the Claude Desktop poller runs inside the trusted Dagster deployment and
ships its conversations through ``/ingest/agent-sessions/batch`` like every other
agent-session source, so it signs and posts exactly as the Go client does. The
app owns every storage detail (folder ids, object keys, ``kind`` values,
``pdw_*`` tags, and the Drive credential); this side holds only the app base URL
and the shared signing key.
