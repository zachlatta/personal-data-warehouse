...
class AppleMessagesUploadStateEntry:
    source_type: str
    source_id: str
    fingerprint: str
    complete: bool
...
class AppleMessagesUploadState:
    ...
    @classmethod
    def open(cls, path: Path, *, account: str, store_path: Path | str) -> AppleMessagesUploadState:
        return cls(path=path, account=account, store_path=store_path)

    def close(self) -> None:
        self._connection.close()

    def entry_for(self, *, source_type: str, source_id: str) -> AppleMessagesUploadStateEntry | None:
        ...
        if row is None:
            return None
        return AppleMessagesUploadStateEntry(
...
def default_state_file() -> Path:
    return Path.home() / "Library" / "Application Support" / "personal-data-warehouse" / "apple-messages-upload-state.sqlite"
